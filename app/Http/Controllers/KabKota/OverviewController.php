...
class OverviewController extends Controller
{
    public function index()
    {
        ...
        $total_struktural = DB::table('user_pejabat_strukturals')->where('user_pejabat_strukturals.tingkat_aparatur', '=', 'kab_kota')->where([['user_pejabat_strukturals.kab_kota_id', $kab_kota_id]])->count();
        ...
        $total_analis = DB::table('users')->rightJoin('user_aparaturs', 'user_aparaturs.user_id', '=', 'users.id')->join('role_user', 'role_user.user_id', '=', 'users.id')->whereRaw('role_user.user_id > 4 and role_user.user_id < 8')->where('user_aparaturs.tingkat_aparatur', '=', 'kab_kota')->where([['user_aparaturs.kab_kota_id', $kab_kota_id]])->count();
