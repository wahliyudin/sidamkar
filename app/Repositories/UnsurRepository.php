...
class UnsurRepository
{
    ...
    public function getRekapUnsurs(User $user)
    {
        ...
        return DB::select("SELECT laporan_kegiatan_jabatans.id as laporan_jabatan_id,
            ...
            DATE(laporan_kegiatan_jabatans.current_date) as tanggal,
            butir_kegiatans.satuan_hasil,
            COUNT(laporan_kegiatan_jabatans.butir_kegiatan_id) as volume,
            butir_kegiatans.score,
            SUM(laporan_kegiatan_jabatans.score) as jumlah_angka_kredit
            FROM unsurs
